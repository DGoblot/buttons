...
var mousePosition
var doDrag = false
@onready var slider = $"slider/Slider Area/Path3D"
...
func _process(delta):
	if draggingCollider:
		var closest_point = point_on_line(slider.global_transform.basis * slider.curve.get_point_position(0), slider.global_transform.basis * slider.curve.get_point_position(1), mousePosition)
		draggingCollider.position = slider.curve.get_closest_point(mousePosition) * slider.global_transform.basis
...
func point_on_line(line_start : Vector3, line_end : Vector3, point_position : Vector3):
